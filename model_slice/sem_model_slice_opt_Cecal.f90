program sem_model_slice
  ...
  integer, parameter :: NMAXPTS = 11000000
  ...
  integer :: nspec,nglob,nspec_irregular_n
  ...
  double precision :: Xmin=609836.823950431,Xmax=1012890.43242007
  double precision :: Ymin=3845964.21988325,Ymax=4166167.00554831
  double precision :: xlen,ylen
  integer :: nproc_xi=10,nproc_eta=8,rec_slice_num
  !!!
...
!!! Kai added
  xlen=(Xmax-Xmin)/nproc_xi
  ylen=(Ymax-Ymin)/nproc_eta
!!!
 do ipt=1,npts
  do ispec=1,nspec
      ...
      do j=1,NGLLY
        do i=1,NGLLX
          ...
          dist(ipt)=dsqrt((x(ipt)-dble(xstore(iglob)))**2 &
            ...
            rec_slice_num=int((x(ipt)-Xmin)/xlen)+int((y(ipt)-Ymin)/ylen)*nproc_xi
            if(rec_slice_num.ne.myrank) then
              distmin(ipt)=HUGEVAL
              ispec_min(ipt)=ispec
...
!                 ispec_min(i),ix_min(i),iy_min(i),iz_min(i),v(i))
  ...
  if (myrank == 0) then
    ...
    if (TOPOGRAPHY) then
      topo_file=trim(TOPO_FILE_SOCAL)
