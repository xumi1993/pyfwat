program sem_model_addpert
  ...
  !====================================================
  integer :: ier,sizeprocs,myrank,ios, i,j, k, ispec,iglob,ipt, npts, nspec_irregular_n
...
!  real(kind=CUSTOM_REAL),dimension(NGLOB_AB) ::  xstore,ystore,zstore
  ...
  character(len=50) :: s_sigma,s_pert
  double precision :: sigma,pert,gaus
  !!!
...
!            endif
 ...
 enddo
  ...
  write(prname,'(a,i6.6,a)') trim(model_dir)//'/proc',myrank,'_'
  local_data_file = trim(prname) // trim(data_name)//'_new.bin'
  open(unit = 27,file=local_data_file, iostat = ios,form ='unformatted')
  if (ios /= 0) call exit_mpi(myrank, 'Error reading model data file')
  write(27) vstore_new
  close(27)
  write(prname,'(a,i6.6,a)') trim(model_dir)//'/proc',myrank,'_'
  local_data_file = trim(prname) //'dlnvs_gaus.bin'
  open(unit = 27,file=local_data_file, iostat = ios,form ='unformatted')
  if (ios /= 0) call exit_mpi(myrank, 'Error reading model data file')
  write(27) dlnvs_gaus
  close(27)
