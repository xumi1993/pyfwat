!!!!
! This program is to add checkboard anomalies to existing model
! Author: Kai Wang, Univeristy of Toronto
! Date  : Aug 7, 2019
...
!  integer, dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: ibool
  ...
  character(len=500) :: xyz_infile,topo_dir,model_dir,data_name,gmt_outfile,&
             local_data_file, prname, out_dir
  real(kind=CUSTOM_REAL),dimension(NMAXPTS) :: x, y, z, v, pert, vmin, &
...
!  real(kind=CUSTOM_REAL),dimension(NGLOB_AB) ::  xstore,ystore,zstore
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: vstore_new
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: dlnvs_gaus
  integer :: nspec,nglob
  ...
  logical, parameter :: TOPOGRAPHY = .false.

  character(len=100) :: topo_file
  integer, dimension(NX_TOPO_SOCAL,NY_TOPO_SOCAL) :: itopo_bathy_basin
  double precision :: elevation
  !!! Kai added
  ! for find slice number of receiver
  character(len=50) :: s_sigma
  double precision :: sigma,gaus
  !!!
  ...
  call getarg(4,out_dir)
  call getarg(5,data_name)
  call getarg(6,s_sigma)
  ...
  do while (1 == 1)
    i=i+1
    read(11,*,iostat=ios) x(i),y(i),z(i),pert(i)
    if (ios /= 0) exit
  enddo
  close(11)
  npts=i-1
  ...
  open(unit = 27,file = trim(prname)//'external_mesh.bin',status='old',action='read', iostat = ios,form ='unformatted')
  if (ios /= 0) call exit_mpi(myrank,'Error reading local x file')
  read(27) nspec
  read(27) nglob
  allocate(ibool(NGLLX,NGLLY,NGLLZ,nspec))
  allocate(vstore(NGLLX,NGLLY,NGLLZ,nspec))
  allocate(vstore_new(NGLLX,NGLLY,NGLLZ,nspec))
  allocate(dlnvs_gaus(NGLLX,NGLLY,NGLLZ,nspec))
...
!          vstore_new(i,j,k,ispec)=vstore(i,j,k,ispec)
!          dlnvs_gaus(i,j,k,ispec)=0.
           if(dist(ipt) <= sigma) then
              dlnvs_gaus(i,j,k,ispec)=dlnvs_gaus(i,j,k,ispec)+pert(ipt) !*gaus
           endif
        enddo
      enddo
    enddo
  ...
  write(prname,'(a,i6.6,a)') trim(out_dir)//'/proc',myrank,'_'
  local_data_file = trim(prname) // trim(data_name)//'.bin'
  open(unit = 27,file=local_data_file, iostat = ios,form ='unformatted')
  if (ios /= 0) call exit_mpi(myrank, 'Error reading model data file')
  write(27) vstore_new
  close(27)
  write(prname,'(a,i6.6,a)') trim(out_dir)//'/proc',myrank,'_'
  local_data_file = trim(prname)// trim(data_name)//'_pert.bin'
  open(unit = 27,file=local_data_file, iostat = ios,form ='unformatted')
  if (ios /= 0) call exit_mpi(myrank, 'Error reading model data file')
  write(27) dlnvs_gaus
  close(27)
  ...
  deallocate(ibool)
  deallocate(vstore)
  deallocate(vstore_new)
  deallocate(dlnvs_gaus)
  deallocate(xstore)
  deallocate(ystore)
  deallocate(zstore)
